...
class BaseTrainer:
    ...
    def _resume_checkpoint(self):
        ...
        if latest_model_path.exists():
            print(f"Loading the latest checkpoint in {latest_model_path}.")
            ...
            print(f"Checkpoint loaded. Training will begin in the {self.start_epoch} epoch.")
        else:
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.save_checkpoint_interval != 0 and epoch % self.save_checkpoint_interval== 0:
                print(f"[{timer.duration()} seconds] Training over, saving...")
                self._save_checkpoint(epoch)

            if self.validation_interval != 0 and epoch % self.validation_interval == 0:
                print(f"[{timer.duration()} seconds] Validation in progress...")
            ...
            print(f"[{timer.duration()} seconds] End of epoch.")
